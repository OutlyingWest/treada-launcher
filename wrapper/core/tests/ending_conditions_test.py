...
from wrapper.core.ending_conditions import StepBasedEndingCondition, LineEndingCondition


class LineEndingConditionTests(unittest.TestCase):
    def setUp(self) -> None:
        self.config = load_config('config.json')
        # self.cdn = LineEndingCondition(precision=0.001,
        #                                     chunk_size=3,
        #                                     big_step_multiplier=1,
        #                                     low_step_border=5)

        self.cdn = LineEndingCondition(precision=1e-2,
                                       chunk_size=100,
    ...
    def test_check(self):
        ...
        for x, y in zip(self.X, self.Y):
            # for chunks in self.cdn.ping_pong:
            ...
            # print(f'{x, y}')
            # print(f'{self.cdn.ping_pong=}')
            # print(self.cdn.ping_pong)

            condition = self.cdn.check(current_index=x, source_current=y)
    ...
    def test_speed_check(self):
        ...
        for x, y in zip(self.X, self.Y):
            self.cdn.check(current_index=x, source_current=y)
        end_time = time.time()
