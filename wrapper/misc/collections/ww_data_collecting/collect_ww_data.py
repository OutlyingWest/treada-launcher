...
def main():
    ...
    ww_data_collector = WWDataCollector(ww_descriptions_path)
    # print(f'{ww_data_collector.descriptions}')
...
class WWDataCollector:
    def __init__(self, descriptions_path: str):
...
class WWDataCmdUserInteractor:
    def __init__(self, ww_data_collector: WWDataCollector, paths):
        ...
        self.ww_number_to_show = None
        self.paths = paths
        self.stage_folder_path = None
        ...
        while running_flag:
            ...
            try:
                ...
                if cmd_code >= 0:
                    ...
                    all_ww_dir_numbers_list = self._list_stage_dir_indexes(stage_name)
                    ww_dir_numbers_range = self._input_ww_dir_numbers_range(all_ww_dir_numbers_list)
                    if stage_name and ww_dir_numbers_range:
                        ww_dir_numbers_list = self._ww_range_to_list(ww_dir_numbers_range, all_ww_dir_numbers_list)
                        ww_dict = self.data_collector.load_ww_data(abs_res_path=self.paths.result.temporary.distributions,
                        ...
                        # print(f'{ww_dict=}')
                        ww_data_plotter = WWDataPlotter(ww_dict)
                        ww_data_plotter.set_plot_axes_labels(x_label='x', y_label=ww_description)
                        if df_col_name:
                            ww_data_plotter.set_plot_title(df_col_name)
                            ww_data_plotter.set_window_title(df_col_name)
                        ww_data_plotter.show()
    ...
    def _input_stage_name(self):
        ...
        stage_name = input()
        if stage_name in os.listdir(self.paths.result.temporary.distributions):
            return stage_name
        else:
            print('Wrong WW dir name')
            return None

    def _list_stage_dir_indexes(self, stage_name: str) -> list:
        stage_folder_path = os.path.join(self.paths.result.temporary.distributions, stage_name)
    ...
    def _input_ww_dir_numbers_range(self, stage_dir_indexes: list):
    ...
    def _ww_range_to_list(self, ww_range: list, stage_data_indexes: list) -> List[int]:
